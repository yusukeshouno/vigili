...
`ask` と判定された確認を「どこで人間に出すか」をユーザーが切り替えられる。状態は daemon が単一の真実として持ち（`~/.vigili/ask-mode`、デフォルト `integrated`）、全クライアント（Mac メニューバー / iPhone / PWA）にトグルを置いて WS で同期する。

| | `integrated`（統合・デフォルト） | `native-first`（Claude アプリ優先） |
|---|---|---|
| policy の allow / deny | 効く | **同じく効く**（自動承認の高速化・invariant の強制 deny は維持） |
| `ask` の行き先 | Vigili に表示 → `timeout_seconds` 無応答でネイティブにフォールバック | **即 `fallback`** → Claude Code のネイティブ確認が最初から出る |
| Vigili の pending 表示 | 出す | 出さない（履歴・統計には `fallback` として記録） |

- 切り替えは WS クライアントメッセージ `{type:"set-ask-mode", mode}` で行い、daemon は `{type:"ask-mode", mode}` を全クライアントに broadcast する。snapshot にも `ask_mode` を同梱する
- `native-first` 中も gate → daemon の経路・判定ログは維持される。変わるのは ask の人間ルーティングのみ
- ユースケース: Mac の前で作業中はネイティブ確認の方が視線移動が少ない（`native-first`）。離席・外出時はスマホに飛ばす（`integrated`）。スマホからも切り替えられることが重要（離席後に「優先モードのままだった」を直せる）
