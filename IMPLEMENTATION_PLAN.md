...
**Done の定義**: `import { ToolRequestSchema } from "@vigili/shared"` が他パッケージから動く。
...
- [x] `packages/relay/` を新設 (Fastify + ws + SQLite / better-sqlite3)
- [x] `/v1/agents/:pid` 認証付き WSS (agent_key、Bearer or `?token=`)
- [x] `/v1/clients/:pid` 認証付き WSS (user_token、同上)
- [x] pairing-id ごとに WS メッセージを fan-out + `agent-status` broadcast
- [x] signup / signin / signout / me / pairings / devices REST API
- [x] vitest 29 本 (DB / auth / hub / 統合 REST + WSS)
- [x] VPS デプロイ (systemd + nginx + Let's Encrypt) — `https://relay.vigili.io` 稼働中
