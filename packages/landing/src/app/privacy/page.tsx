    ...
    >
      <Block title="What the Vigili daemon sees">
        <p>
          ...
          input to each Claude Code tool call — the shell command, file path, or URL —
          plus a session identifier and current working directory. It does not see your
          conversation with Claude, Claude&apos;s responses, or the contents of files
          Claude reads.
        </p>
      </Block>
      <Block title="What stays local">
        <p>
          ...
          Policy rules live in <Code>~/.vigili/policy.yaml</Code>. Both are owner-only
          (file mode 0600) and never leave your machine unless you choose to use the cloud
          relay.
        </p>
      </Block>
      <Block title="What the optional cloud relay sees">
        <p>
          If you enable the managed relay at <Code>relay.vigili.io</Code> for outside-LAN
          access, the relay routes by pairing-id and sees the same approval payload your
          phone sees (the tool input, e.g. the command line being run). It does not store
          this payload — it only forwards in memory. End-to-end encryption between Mac
          and phone is on the roadmap.
        </p>
        <p>
          You can also self-host the relay — the code is OSS. In that case Vigili.io sees
          nothing.
        </p>
      </Block>
      <Block title="Waitlist">
        <p>
          If you enter your email on the landing page, we store it solely to send you a
          single notification when Vigili launches. No newsletters, no marketing, no
          third-party sharing. Unsubscribe by replying to that email or emailing
          <Code>privacy@vigili.io</Code> any time.
        </p>
      </Block>
      <Block title="Analytics">
        <p>
          Page visits are measured via Vercel Analytics in cookieless mode. We see
          aggregate counts (visits, referrers, browsers) but no personal identifiers
    ...
    >
      ...
      <Block title="ローカルに留まるデータ">
        <p>
          ...
          <Code>~/.vigili/policy.yaml</Code> に置かれます。どちらも所有者のみ読み書き可能
          (0600) で、クラウド relay を利用しない限り端末外に送信されません。
        </p>
      </Block>
      <Block title="クラウド relay (任意) が見るもの">
        ...
        <p>
          relay は OSS なので自前運用も可能です。その場合 Vigili.io は何も見ません。
        </p>
      </Block>
      ...
      <Block title="解析ツール">
        <p>
          Vercel Analytics の cookieless モードでページ訪問を計測しています。訪問数 /
          リファラ / ブラウザの集計データのみで、個人を識別する情報やサイトを跨いだ
          トラッキングは取得しません。
        </p>
      </Block>
      <Block title="お問い合わせ">
