...
> **2026-06 プロダクト化への転換**: 旧「一人開発者専用」前提を見直し、**マルチアカウント対応**（Sign in with Apple によるアカウント中心オンボーディング）と **GUI オンボーディング**（ターミナル不要のサインイン・hook 自動導入）を目標に格上げした。relay (`relay.vigili.io`) は Vigili が運用する単一クラウドとして正式採用（旧「外部公開は Tailscale Funnel のみ」は撤回）。ローカル daemon は引き続き各ユーザーの PC 常駐で、クラウドには出さない。下記は引き続きやらない。

- **GUI でのポリシー編集**。ルール (policy.yaml) は YAML 直書きが正。アプリ/PWA からはルール昇格のみ。
- **Claude Code 以外のエージェント対応**。OpenAI Codex などへの拡張は後回し。
- **セルフホスト relay**。relay は Vigili 運用の単一クラウドのみ。各自で relay を立てる構成はサポートしない。
- **Mac/iOS 以外のネイティブクライアント**。Android などは PWA で代替する。
...
- gate がデーモンに接続できなかった場合のフェイルセーフは **ネイティブ確認フローへの素通し**（無出力 exit 0）。gate が自動許可することは決してないが、全 deny で作業を止めることもしない — Claude Code 本体の標準プロンプトが人間に確認する（SPEC §2.5）。守るべき不変条件は「daemon 不在時に *無確認で* 実行される操作を増やさない」こと。
